...
def main(args: argparse.Namespace):
    ...
    for epoch in range(args.epochs):
        ...
        mem_fea_target, mem_cls_target = train(dset_loaders,  train_source_iter, train_target_iter, classifier, domain_adv, optimizer,
              lr_scheduler, epoch, args, mem_fea_target, mem_cls_target, mtkr)

        # evaluate on validation set
        acc1 = utils.validate(val_loader, classifier, args, device)
        calculate_confidence_stats(dset_loaders["target"], classifier, device)
...
def train(dset_loaders, train_source_iter, train_target_iter,
          ...
          lr_scheduler: LambdaLR, epoch: int, args: argparse.Namespace, mem_fea_target, mem_cls_target, mtkr):


    losses = AverageMeter('Loss', ':6.2f')  
    cls_accs = AverageMeter('Cls Acc', ':6.1f')  
    tgt_accs = AverageMeter('Tgt Acc', ':6.1f')  
    tgc_accs = AverageMeter('Tgc Acc', ':6.1f')  
    progress = ProgressMeter(
        args.iters_per_epoch,
        [losses, cls_accs, tgt_accs,tgc_accs],
    ...
    for i in range(args.iters_per_epoch):
        ...
        cls_acc = accuracy(y_s, labels_s)[0]  # 源域分类准确率
        tgt_acc = accuracy(y_t, labels_t)[0]  # 目标域分类准确率
        tgc_acc = accuracy(kpred, labels_t.cpu())[0]  # 目标域分类准确率
        

        losses.update(mtkr_loss.item(), x_s.size(0))
        cls_accs.update(cls_acc.item(), x_s.size(0))
        tgt_accs.update(tgt_acc.item(), x_t.size(0))
        tgc_accs.update(tgc_acc.item(), x_t.size(0))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--entropy_weight', type=float, default=0.3)
    parser.add_argument('--consistency_s', type=float, default=1.0)
    parser.add_argument('--consistency_t', type=float, default=0.2)
    parser.add_argument('--p_threshold', type=float, default=0.3)
